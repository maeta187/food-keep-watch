...
- `docs/development-roadmap.md` を参照して開発を進めてください。
...
- データベースは Drizzle ORM と `expo-sqlite` を組み合わせ、端末内ストレージで管理する。
- データ保存方式は端末内保存を中心とし、クラウド連携が必要になった場合は検討内容と判断理由を記録する。
...
- 関数コンポーネントの返り値に `JSX.Element` を明示するために `React` から `JSX` を import する必要はないので、返り値型は基本的に推論に任せます。
...
- 単体テストは `bun run test`（または `bun run test:watch`）で実行し、方針は `docs/testing-guidelines.md` を参照してください。

## ドキュメントとコメント

- 重要なビジネスロジックには意図を簡潔に説明するコメントを付記してください。
- ユーザーに見える挙動が変化した場合は README やアプリ内ドキュメントを更新してください。
- 末端のユーティリティ関数や外部公開する関数には、処理概要・引数の型を記した JSDoc コメントを付けてください（返り値の型は不要、説明文と各タグの間に空行を入れる）。
...
- コミット時にいかなる場合でも差分に変更を入れないでください。
- もし修正が必要な場合はコミットを中止そのことを開発者に報告してください。
